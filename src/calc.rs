...
pub unsafe fn trapezoid_init_triangle(trap: &mut [Trapezoid; 2], mut p1: Vertex,
    ...
    if p1.pos.y == p2.pos.y {
        ...
        trap[0].right.v1 = p2;
        trap[0].right.v2 = p3;
        return match trap[0].top < trap[0].bottom {
            true => { 1 }
            false => { 0 }
        };
    }
    ...
    let k = (p3.pos.y - p1.pos.y) / (p2.pos.y - p1.pos.y);
    let x = p1.pos.x + (p2.pos.x - p1.pos.x) * k;
    if x <= p3.pos.x {
        trap[0].left.v1 = p1;
        trap[0].left.v2 = p2;
        trap[0].right.v1 = p1;
...
pub fn trapezoid_init_scan_line(trap: &mut Trapezoid, scanline: &mut Scanline, y: i32) {
